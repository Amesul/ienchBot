...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("contexte")
        .setDescription(
          "Le projet, lieu, instant où a été prononcée la citation"
        )
        .setRequired(false)
    ),
  async execute(bot, interaction, database) {
    ...
    database.collection("quotes").insertOne({
      ...
      contexte: interaction.options.get("contexte")
        ? interaction.options.get("contexte").value
        : "",
    ...
    const quoteEmbed = new MessageEmbed()
      ...
      .setDescription(
        `***${interaction.options.get("quote").value}*** - ${
          interaction.options.get("name").value
        }\n ${
          interaction.options.get("contexte")
            ? interaction.options.get("contexte").value
            : ""
        }`
      );
